...
model = load_model(BASE_DIR / 'models_pred/bj_model.h5')
...
def alert():
    data = yf.download(tickers='BJ', period='30d', interval='1d')
    ...
    data['month'] = month
    data['day'] = day
    dataset = data.loc[:, ['Open', 'High', 'Low',
                           'Close', 'Volume', 'DoW', 'month', 'day']]
...
def pattern():
    data = yf.download(tickers='BJ', period='200d', interval='1d')
